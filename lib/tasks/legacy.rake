...
namespace :legacy do
  ...
  namespace :import do
    ...
    task :users, [:file] => :environment do |_t, args|
      ...
      csv.each do |row|
        ...
        if user.nil?
          role = company.roles.find_by_name(row['Group name']) or fail "Role '#{row['Group name']}' not found"
          user_info = {
            email: row['Email'],
    ...
    task :assign_campaigns, [:file] => :environment do |_t, args|
      args.with_defaults(file: "#{Rails.root}/db/users.csv")
      csv_text = File.read(args.file)
      csv = CSV.parse(csv_text, headers: true)
      company = Company.find_by_name('Legacy Marketing Partners')
