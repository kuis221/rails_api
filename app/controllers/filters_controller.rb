...
class CollectionFilter
  ...
  SETTINGS = YAML.load_file(File.join(Rails.root, 'config', 'filters.yml')).tap do |settings|
    settings['filters'].each do |collection, config|
      config.each do |bucket, info|
        if info.is_a?(Array)
          config[bucket] = {
            'classes' => info.map{ |model| model.constantize },
  ...
  def filters
    SETTINGS['filters'][scope].map do |bucket_name, bucket_config|
      build_filter_bucket bucket_name, bucket_config
    end.flatten.append user_saved_filters
  end

  private

  def build_filter_bucket(name, config)
    if config.key?('method')
      self.send(config['method'])
    else
