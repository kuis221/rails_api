class EventsController < FilteredController
  ...
  respond_to :js, only: [:new, :create, :edit, :update, :edit_results, :save_results, :edit_summary]

  custom_actions member: [:tasks, :edit_results, :save_results, :edit_summary]
  ...
  protected
    ...
    def facets
      @facets ||= Array.new.tap do |f|
        ...
        f.push(label: "Status", items: ['Active', 'Inactive', 'Late', 'Submitted', 'Rejected', 'Approved'].map{|x| build_facet_item({label: x, id: x, name: :status, count: 1}) })
      end
    end
