  ...
  def collection_to_csv
    ...
    CSV.generate do |csv|
      csv << [
        ...
        'SUBMITTED AT', 'APPROVED AT', 'CREATED AT', 'CREATED BY', 'LAST MODIFIED', 'MODIFIED BY'].concat(
          exporter.expenses_columns)
      each_collection_item do |event|
        csv << [
          event.campaign_name, event.place_name, event.place_address, event.start_date, event.end_date,
          event.submitted_date, event.approved_date, event.created_at, event.created_by, event.last_modified,
          event.modified_by, event.country].concat(exporter.event_expenses(event))
      end
    end
  end
