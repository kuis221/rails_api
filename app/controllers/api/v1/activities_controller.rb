  ...
  def custom_field_values(field, result)
    if field.type == 'FormField::Percentage'
      { segments: (field.options_for_input.map do|s|
                    { id: s[1], text: s[0], value: result ? result.value[s[1].to_s].to_i : nil, goal: (field.kpi_id.present? && resource.kpi_goals.key?(field.kpi_id) ? resource.kpi_goals[field.kpi_id][s[1]] : nil) }
                  end) }
