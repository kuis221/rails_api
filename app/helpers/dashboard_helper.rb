module DashboardHelper
  ...
  def upcoming_events_list
    @upcoming_events_list ||= Event.do_search(company_id: current_company.id, current_company_user: current_company_user, per_page: 5, sorting: :start_at, sorting_dir: :asc, start_date: [Time.zone.now.strftime('%m/%d/%Y')], end_date: [(Time.zone.now + 10.years).strftime('%m/%d/%Y')], event_status: ['Active']).results
  end
  ...
  def campaign_promo_hours_chart(c)
    ...
    content_tag(:div, class: 'chart-bar') do
      ...
      content_tag(:div, content_tag(:div, "<b>#{number_with_precision(c['goal'], strip_insignificant_zeros: true, delimiter: ',')}</b> GOAL".html_safe), class: 'goal-label') +       content_tag(:div, class: 'remaining-label' + (c['remaining'] < 0 ? ' over-goal' : '')) do
