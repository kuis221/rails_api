...
module Analysis
  module CampaignsReportHelper
    ...
    def vertical_progress_bar(complete, today)
      complete = 1 if complete < 1 && complete > 0
    ...
    def reach_narrative(data)
      ...
      max_ethnicity = data['ethnicity'].max_by { |_k, v| v }.first   if data['ethnicity'].values.max > 0
      max_age = data['age'].max_by { |_k, v| v }.first               if data['age'].values.max > 0
      max_gender = data['gender'].max_by { |_k, v| v }.first         if data['gender'].values.max > 0
    ...
    def awareness_narrative(statistics)
      if statistics.present? && statistics['aware'].present?
        i = -1
        rankings = %w(first second third fourth fifth)
        statistics['aware'].sort_by { |_brand, data| -data[:avg] }.map do |k, v|
    ...
    def conversion_narrative(statistics)
      ...
      if statistics.present? && statistics['5'].present? && statistics['5'].any?
        statistics['5'].sort_by { |_brand, data| -data[:avg] }.map do |k, v|
        ...
        statistics['4'].sort_by { |_brand, data| -data[:avg] }.map do |k, v|
