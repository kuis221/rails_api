module UsersHelper
    ...
    Rails.cache.fetch("user_notifications_#{user.id}", expires_in: 15.minutes) do
      ...
      if can?(:view_list, Event)
        ...
        if status_counts[:due] > 0 && user.allow_notification?('event_recap_due_app')
          alerts.push(message: I18n.translate('notifications.event_recaps_due', count: status_counts[:due]),
        ...
        if status_counts[:late] > 0 && user.allow_notification?('event_recap_late_app')
          alerts.push(message: I18n.translate('notifications.event_recaps_late', count: status_counts[:late]),
        ...
        if status_counts[:submitted] > 0 && user.allow_notification?('event_recap_pending_approval_app')
          alerts.push(message: I18n.translate('notifications.recaps_prending_approval', count: status_counts[:submitted]),
        ...
        if status_counts[:rejected] > 0 && user.allow_notification?('event_recap_rejected_app')
          alerts.push(message: I18n.translate('notifications.rejected_recaps', count: status_counts[:rejected]),
      ...
      if can?(:index_my, Task) && user.allow_notification?('late_task_app')
        ...
        if count > 0
          alerts.push(message: I18n.translate('notifications.task_late_user', count: count), level: 'red',
      ...
      if grouped_notifications['new_event'].present? && grouped_notifications['new_event'].to_i > 0 && can?(:view_list, Event) && user.allow_notification?('new_event_team_app')
        alerts.push(message: I18n.translate('notifications.new_events', count: grouped_notifications['new_event'].to_i), level: 'grey',
      ...
      if grouped_notifications['new_team_event'].present? && grouped_notifications['new_team_event'].to_i > 0 && can?(:view_list, Event) && user.allow_notification?('new_event_team_app')
        ...
        alerts.push(message: I18n.translate('notifications.new_team_events', count: team_ids.count, teams_names: team_names, events_sentence: events_sentence), level: 'grey',
      ...
      if grouped_notifications['new_campaign'].present? && grouped_notifications['new_campaign'].to_i > 0 && can?(:read, Campaign) && user.allow_notification?('new_campaign_app')
        alerts.push(message: I18n.translate('notifications.new_campaigns', count: grouped_notifications['new_campaign'].to_i), level: 'grey',
      ...
      if grouped_notifications['new_task'].present? && grouped_notifications['new_task'].to_i > 0 && can?(:index_my, Task) && user.allow_notification?('new_task_assignment_app')
        alerts.push(message: I18n.translate('notifications.new_tasks', count: grouped_notifications['new_task'].to_i), level: 'grey',
  ...
  def custom_filter_setting_checkbox(type, subject, label, filter_settings)
    field_id = "#{type.gsub(/\s+/, '_').downcase}_#{subject}_#{label.downcase}"
    content_tag(:label) do
      check_box_tag('filter_setting[settings][]', field_id, filter_settings.persisted? && filter_settings.present? ? filter_settings.settings.include?(field_id) : ('Active' == label), id: "settings_#{field_id}") + label
    end
  end
end
