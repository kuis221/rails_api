class CalculationInput < SimpleForm::Inputs::Base
  def input(_wrapper_options)
    ...
    options[:collection].each_with_index do |ffo, i|
      ...
      extra_attributes = i > 0 ? "data-operation=\"#{operator(options[:operation])}\"" : ''
    ...
    output_html << '<tr class="field-option calculation-total-field" data-field-id="' + field_id + '"><td colspan="2" class="text-right">'
