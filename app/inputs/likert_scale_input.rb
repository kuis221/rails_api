    ...
    options[:statements].map do |statement|
      '<tr><td><label class="likert-statement">' + statement.name + '</label></td>' + options[:collection].map do |option|
        ...
        if options[:capture_mechanism] == 'radio' # Single Answer
          ...
          field_name = "#{object_name}[#{attribute_name}][#{statement.id}][]"
          '<td><label class="checkbox multiple"><div class="checkbox"><input type="checkbox" name="' + field_name + '" id="' + field_id + '_' + option.id.to_s + '" value="' + option.id.to_s + '"' + checked + '></div></label></td>'
        end
      end.join + '</tr>'
