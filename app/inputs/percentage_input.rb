  ...
  def input
    if options[:collection] && options[:collection].length > 0
      ...
      options[:collection].each do |ffo|
        ...
        output_html << "<div class=\"input-append\">#{@builder.text_field(nil, input_html_options.merge(name: field_name, id: field_id, value: values.try(:[], ffo.id.to_s)))}<span class=\"add-on\">%</span></div>" +
                       "<label for=\"#{field_id}\">#{ERB::Util.html_escape(ffo.name)}</label>"
      end
