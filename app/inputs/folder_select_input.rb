class FolderSelectInput < SimpleForm::Inputs::Base
  ...
  def document_folder_tree(folder, children, list_class=nil)
    template.content_tag(:ul, id: "folder-#{folder.id}", class: "folder-contents #{list_class}") do
      children.map do |sub_folder|
        ...
        template.content_tag(:li, class: 'subfolder '+(has_children ? 'with-children' : ''.html_safe)) do
          (has_children ? template.tag(:i, class: 'icon icon-arrow-right folder-arrow-indicator', data: {toggle: 'collapse', target: "#folder-#{sub_folder.id}"}) : '').html_safe +
          ...
          (has_children ? document_folder_tree(sub_folder, sub_folder.document_folders, 'collapse') : '')
        end
      end.join.html_safe
    end
  end
