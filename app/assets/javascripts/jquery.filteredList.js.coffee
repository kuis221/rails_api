...
$.widget 'nmk.filteredList', {
	options: {
		...
		emptyStateMessage: 'There are no results matching the filtering criteria you selected.',
	...
	_placeholderEmptyFilterResults: () ->
		...
		line2 = @options.emptyFilterResultsMessageLine2
		message = "<p>#{line1}<br />#{line2}</p>"
		...
		@jqxhr = $.ajax
			...
			success: (response, textStatus, jqXHR) =>
				...
				if typeof response is 'object'
					...
					resultsCount = $items.data('total')
					totalItemsCount = $items.data('existing-total')

					if page is 1
						if totalItemsCount is 0
							@emptyState = @_placeholderEmptyState()
						else if resultsCount is 0
							@emptyState = @_placeholderEmptyFilterResults()
