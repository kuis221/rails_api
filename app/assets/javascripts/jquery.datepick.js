  ...
  $.extend(Datepicker.prototype, {
    ...
    _showMonthPlugin: function (target, year, month, day) {
      ...
      if (inst && (day != null ||
        ...
        inst.drawDate = plugin.newDate(show.getFullYear(), show.getMonth() + 1, (day != null ? day : Math.min(inst.drawDate.getDate(),
    ...
    _highlightDates: function (inst, target, elem1, elem2) {
      ...
      if (inst.selectedDates[0] != date1 || inst.selectedDates[1] != date2) {
        ...
        target.picker.find(inst.options.renderer.daySelector + ' a').each(function (index, element) {
          $(element).removeClass('first-selected last-selected');
          if (elem1 == element) {
            $(element).addClass('first-selected');
            insideRange = true;
          }
          if (insideRange) {
            $(element).addClass('datepick-selected');
          } else {
            $(element).removeClass('datepick-selected');
          }
          if (elem2 == element) {
