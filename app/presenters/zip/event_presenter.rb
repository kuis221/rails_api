module Zip
  class EventPresenter < Csv::EventPresenter
    def receipts_for_zip_export
      @model.event_expenses.includes(:creator).each_with_index.map do |expense, index|
        next unless expense.receipt.present?
        file_local_name = "#{Rails.root}/tmp/#{expense.id}-#{Time.now.to_f}-#{rand(1000)}"
        ...
        [sanitize_filename(generate_filename(expense, index)), file_local_name]
      end.compact
    end
