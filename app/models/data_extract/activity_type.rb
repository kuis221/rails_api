...
class DataExtract::ActivityType < DataExtract
  define_columns name: 'name',
                 ...
                 created_by: '(SELECT trim(us.first_name || \' \' || us.last_name) FROM users as us WHERE activity_types.created_by_id=us.id)',
                 modified_at: proc { "to_char(activity_types.updated_at, 'MM/DD/YYYY')" },
                 modified_by: '(SELECT trim(us.first_name || \' \' || us.last_name) FROM users as us WHERE activity_types.updated_by_id=us.id)',
