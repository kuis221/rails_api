...
#
class DataExtract::User < DataExtract
  ...
  teams_name: 'array_to_string(ARRAY(SELECT teams.name FROM teams 
              ...
              WHERE company_users.id=memberships.company_user_id 
              AND company_users.user_id=users.id ),\', \') AS teams_name', 
  email: 'users.email', 
  ...
  def total_results
    User.connection.select_value("SELECT COUNT(*) FROM (#{base_scope.select(*selected_columns_to_sql).to_sql}) sq").to_i
  end
end