module DataExtractEventsBase
  ...
  def add_filter_conditions_to_scope(s)
    ...
    s = s.joins('LEFT JOIN venues ON venues.place_id = events.place_id and venues.company_id = events.company_id')
          .where('venues.id IN (?)', filters['venue']) if filters['venue'].present?
    s
  end
