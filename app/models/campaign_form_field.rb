...
class CampaignFormField < ActiveRecord::Base
  ...
  def field_options(result)
    options = {as: simple_form_field_type, capture_mechanism: self.capture_mechanism, label: self.name, field_id: self.id, options: self.options, required: is_required?, input_html: {value: result.value, class: field_validation_classes(result)}}
