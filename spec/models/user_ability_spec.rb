...
describe "User" do
  describe "abilities" do
    ...
    context "when it is NOT a super user" do
      let(:user){ FactoryGirl.create(:company_user,  company: company, role: FactoryGirl.create(:role, is_admin: false), user: FactoryGirl.create(:user,  current_company: company)).user }
      ...
      #
      describe "Event event expenses permissions" do
        it "should be able to deactivate a event expense in a event if has the permission :deactivate_expense on Event" do
          event = FactoryGirl.create(:event, company: company)
          event_expense = FactoryGirl.create(:event_expense, event: event)
          ability.should_not be_able_to(:destroy, event_expense)
        ...
        it "should be able to edit event expense in a event if has the permission :edit_task on Event" do
          event = FactoryGirl.create(:event, company: company)
          event_expense = FactoryGirl.create(:event_expense, event: event)
          ability.should_not be_able_to(:edit, event_expense)
        ...
        it "should be able to list event expense in a event if has the permission :index_expenses on Event" do
          event = FactoryGirl.create(:event, company: company)
          ability.should_not be_able_to(:expenses, event)
          ability.should_not be_able_to(:index_expenses, event)
