...
describe AttachedAsset, search: true do
  it "should search for roles" do
    ...
    AttachedAsset.do_search(company_id: 1, q: "place,#{place.id}").results.should =~ [asset]
    AttachedAsset.do_search(company_id: 1, q: "place,#{place2.id}").results.should =~ [asset2]
