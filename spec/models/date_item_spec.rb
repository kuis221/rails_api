...
describe DateItem, type: :model do
  ...
  it { is_expected.to ensure_inclusion_of(:recurrence_type).in_array(%w(daily weekly monthly yearly)) }
  it { is_expected.not_to allow_value(%w(sunday monday tuesday wednesday thursday friday hello)).for(:recurrence_days) }
  it { is_expected.to allow_value(%w(sunday monday tuesday wednesday thursday friday saturday)).for(:recurrence_days) }
    ...
    describe 'with recurrence' do
      describe 'with period of 1' do
        it 'correctly describes the type for ' do
          date = described_class.new(recurrence: true, recurrence_period: 1, recurrence_type: 'daily')
          expect(date.label).to eq('Every day')
        end
        it 'correctly describes the type for weekly' do
          date = described_class.new(recurrence: true, recurrence_period: 1, recurrence_type: 'weekly')
          expect(date.label).to eq('Every week')
        end
        it 'correctly describes the type for monthly' do
          date = described_class.new(recurrence: true, recurrence_period: 1, recurrence_type: 'monthly')
      ...
      describe 'with period more than one' do
        it 'correctly describes the type for ' do
          date = described_class.new(recurrence: true, recurrence_period: 3, recurrence_type: 'daily')
          expect(date.label).to eq('Every 3 days')
        end
        it 'correctly describes the type for weekly' do
          date = described_class.new(recurrence: true, recurrence_period: 3, recurrence_type: 'weekly')
          expect(date.label).to eq('Every 3 weeks')
        end
        it 'correctly describes the type for monthly' do
          date = described_class.new(recurrence: true, recurrence_period: 3, recurrence_type: 'monthly')
      ...
      describe 'with recurrence days' do
        it 'correctly adds the day to the description' do
          date = described_class.new(recurrence: true, recurrence_days: ['monday'])
          expect(date.label).to eq('On Monday')
        end
        it "correctly adds the days separated by 'AND'" do
          date = described_class.new(recurrence: true, recurrence_days: %w(monday tuesday))
          expect(date.label).to eq('On Monday and Tuesday')
        end
        it "correctly adds the days separated by commma and 'AND'" do
          date = described_class.new(recurrence: true, recurrence_days: %w(monday tuesday wednesday))
          expect(date.label).to eq('On Monday, Tuesday, and Wednesday')
        end
        it 'should ignore empty days' do
          date = described_class.new(recurrence: true, recurrence_days: [''])
      ...
      describe 'with period and recurrence days' do
        it 'describe it correcly' do
          date = described_class.new(recurrence: true, recurrence_type: 'daily', recurrence_period: '3',  recurrence_days: ['monday'])
      ...
      describe 'with dates, period and recurrence days' do
        it 'describe it correcly' do
          date = described_class.new(recurrence: true, start_date: '01/31/2013', end_date: '02/24/2013', recurrence_type: 'weekly', recurrence_period: '3',  recurrence_days: ['monday'])
