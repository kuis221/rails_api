...
tu =  User.create(email: 'test@brandscopic.com', first_name: 'Test', last_name: 'User', password: 'TestPass321', password_confirmation: 'TestPass321', country: 'US', state: 'CA', city: 'San Francisco', invitation_accepted_at: Time.now, confirmed_at: Time.now, invitation_token: nil, time_zone: 'Pacific Time (US & Canada)')
cu = CompanyUser.create(active: true, user_id: tu.id, company_id: c.id, role_id: c.roles.first.id)
cu.save(validate: false) # Make sure it's saved
