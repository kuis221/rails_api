class RenameCostKpiToExpenses < ActiveRecord::Migration
  def up
    ...
    if kpi = Kpi.global.where(name: 'Cost').reorder('id').first
      kpi.update_attributes(name: 'Expenses', kpi_type: 'expenses', description: 'Total expenses related to the event')
